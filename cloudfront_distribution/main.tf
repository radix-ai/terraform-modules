...
Create a CDN that:
   ...
   3. Adds a default cached behavior and ordered cache behaviors for (2) that
      enables caching and compression.
   4. Attaches optional Lambda@Edge pre/post request processing functions.

Note: you may only supply one origin with a `path_pattern` of `null`, because
this module associates the `default_cache_behavior` with that origin and a
CloudFront distribution can have only one default cache behavior [1].
...
locals {
  ...
  zone_id              = "Z1NIDS8WH5H399"
  workspace_domain     = "${local.workspace_url_prefixes[terraform.workspace]}${local.domain}"
  workspace_api_domain = "${local.workspace_url_prefixes[terraform.workspace]}${local.workspace_api_domain}"  
}
...
module "https_certificate" {
  source  = "github.com/radix-ai/terraform-modules//https_certificate"
  providers = { aws = aws.us_east_1 }
  domains = [
    {domain = local.workspace_domain, zone_id = local.zone_id},
    {domain = local.workspace_api_domain, zone_id = local.zone_id}
  ]
}

module "s3_website_bucket" {
  source      = "github.com/radix-ai/terraform-modules//s3_bucket"
  bucket_name = local.workspace_domain
  website     = true
}

module "cloudfront_cdn" {
  source            = "github.com/radix-ai/terraform-modules//cloudfront_distribution"
...
locals {
  origin_settings = {
    # We must use a custom Origin pointing to the Website Hosting endpoint of
    # the S3 bucket, because we want to use S3's default root object handling
    # and not CloudFront's default root object handling [2].
    S3_WEBSITE : {
